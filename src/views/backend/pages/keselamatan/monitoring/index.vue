<template>
  <div :class="device.desktop ? `pa-6`:`home pa-0 grey lighten-4`">
    <v-row>
      <v-col cols="12">
        ...
        <v-card>
          ...
          >
            <v-icon
              small
              class="mr-1 orange--text animate__animated animate__flash animate__infinite"
            >mdi-circle</v-icon> Formulir Pencetakan Renaksi
          </v-toolbar>
          <v-card-text class="mt-2">
            <v-col cols="12">
              <v-select
                ...
                dense
                hide-details
                v-model="formlaporan.record.periode_uuid"
                :items="periodes"
                @change="fetchTahuns"
              >
              </v-select>
            ...
            <v-col cols="12">
              <v-select
                ...
                dense
                hide-details
                v-model="formlaporan.record.tahun_uuid"
                :items="tahuns"
              >
              </v-select>
            </v-col>
            <v-col cols="12">
              <v-select
                ...
                hide-details
                :color="theme.color"
                v-model="formlaporan.record.pilar_uuid"
                :items="pilars"
              >
              </v-select>
            </v-col>
...
export default {
  ...
  methods: {
    ...
    fetchPeriode: async function () {
      try {
        let { data } = await this.http.get("api/combo/periode");
        this.periodes = data;
      } catch (error) {}
    },
    fetchTahuns: async function () {
      try {
        let { data } = await this.http.get(
          "api/combo/tahun/" + this.formlaporan.record.periode_uuid
        );
        this.tahuns = data;
      } catch (error) {}
    },
    ...
    openFormLaporan: function () {
      this.formlaporan.show = true;
    },
    closeFormLaporan: function () {
      this.formlaporan.show = false;
      this.formlaporan.record = {};
    },
    printReport: async function () {
      try {
        this.setLoading({ dialog: true, text: "Generating Report" });
        let { data } = await this.http
          .get(
            "api/keselamatan/renaksi-report-per-tahun/" +
              this.formlaporan.record.tahun_uuid +
              "/" +
              this.formlaporan.record.pilar_uuid
          )
          .then((res) => {
